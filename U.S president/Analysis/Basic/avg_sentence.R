...
# get average sentence length average
...
ggplot(result2, aes(x = type, y = avg, fill = type)) + geom_col(show.legend = T) + geom_text(aes(label = round(avg, 2), vjust = -0.5)) + labs(title = "the average of president speech sentence length(before 1960, after 1960)") + xlab(NULL) + ylab(NULL) + theme_classic() + theme(text = element_text(family = "a", size = 13), plot.title = element_text(hjust = 0.5, size = 17), panel.grid = element_blank(), axis.text.y = element_text(hjust = 1),  legend.position = "bottom")
