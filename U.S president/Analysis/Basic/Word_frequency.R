...
ggplot(top10_total, aes(x = reorder(token, n), y = n, fill = token)) + geom_col(show.legend = F) + coord_flip() + geom_text(aes(label = n), hjust = -0.3) +  theme_classic() + labs(title = "top 10 word in the America president inaugural adress", x = NULL, y = NULL) + theme(text = element_text(family = "a"), plot.title = element_text(hjust = 0.5), panel.grid = element_blank(), axis.text.y = element_text(hjust = 1))
...
ggplot(top10_except, aes(x = reorder(token, n), y = n, fill = token)) + geom_col(show.legend = F) + coord_flip() + geom_text(aes(label = n), hjust = -0.3) +  theme_classic() + labs(title = "top 10 word in the America president inaugural adress", x = NULL, y = NULL) + theme(text = element_text(family = "a"), plot.title = element_text(hjust = 0.5), panel.grid = element_blank(), axis.text.y = element_text(hjust = 1))
